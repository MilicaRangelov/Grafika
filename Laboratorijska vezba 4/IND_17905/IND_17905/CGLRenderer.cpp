...
void CGLRenderer::DrawScene(CDC* pDC)
{
	...
	glClearColor(1.00, 1.00, 1.00, 0.92);
...
void CGLRenderer::DrawFigure(double angle)
{
	...
	DrawCylinder(1, 1.2, 1, 8, { 0.50, 0.75, 1.00 });

	glTranslatef(0, 0.75, 0);
	DrawCylinder(0.5, 1.4, 1.4, 8, { 0.50, 0.75, 1.00 });
	...
	glPushMatrix();
	{
		...
		glTranslatef(0, heightCylinderSphereDif, 0);
		DrawCylinder(this->cylinderH, this->cylinderR, this->cylinderR, this->nSeg, { 1.0, 1.0, 0.0 });
...
void CGLRenderer::DrawCone(double h, double r, int nSeg, COLOR3F color)
{
	double step = 360.0 / nSeg,
		hHalf = h / 2.0,
		factor = M_PI / 180;
...
void CGLRenderer::DrawAxes(double lenght, bool useNegative)
{
	glLineWidth(2);
...
void CGLRenderer::DrawGrid(double width, double height, int nSegW, int nSegH, COLOR3F color)
{
